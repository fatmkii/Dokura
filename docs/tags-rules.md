...
> 首版范围：仅解析`.zip`文件的文件名，不从父文件夹推断，不使用外部数据库或内容哈希，也不提供人工修正。首版可分类tag只有作者、原作和语言；版本、发布组及其他内容统一保存为未分类tag。本文提到的其他压缩格式、更多tag分类、人工修正、外部元数据、哈希去重和字段锁定均为未来建议，不属于首版需求；首版范围以`docs/PRD.md`为准。
...
说明：仅凭文件名无法确定“毛玉丸”是社团还是个人作者。上述结果只是较合理的默认推断，因此必须保留 `creator_raw`。外部数据库覆盖不属于首版范围。
...
4. 不因解析失败拒绝生成缩略图；
5. 首版在文件名变化或解析器升级后重新解析；外部数据库和人工操作属于未来功能；
6. 首版没有人工修正字段，重新解析时完整替换旧的文件名解析结果。

---

## 14. 未来的数据库字段来源策略（不属于首版）
...
- 原作候选提取；
- 中文、英文、日文语言标签；
- 作者块同时包含社团名和作者名时只生成作者tag，仅有社团名时以社团名作为作者tag；
- 版本、发布组和其他内容作为未分类tag保留；
- 置信度和警告；
- 单元测试。
